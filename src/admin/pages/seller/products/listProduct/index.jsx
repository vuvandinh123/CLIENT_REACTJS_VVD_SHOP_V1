...
const ListProduct = () => {
  ...
  const { data, loading } = useApiCall(async () => {
    ...
    setPagination({
      ...
      totalProduct: response.options?.countProduct,
    });
  ...
  const {
    ...
    fnDeleteMultiple: async ({ listId }) => {
      const res = await deleteProduct({ listId });
      return res;
    },
    fnChangeStatusMultiple: async ({ listId, value }) => {
      const res = await changeStatusProduct({ listId, value });
      return res;
    },
    refresh,
  });
  ...
  return (
    <section>
      {/* header */}
      <PageHeader
        title={t("admin.listProduct.title")}
