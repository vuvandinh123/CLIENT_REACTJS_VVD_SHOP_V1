...
const CountDown = ({ end_time }) => {
  ...
  return (
    <div className="border mt-3  bg-[#F9F2F2] flex flex-col md:flex-row gap-2 justify-between items-center border-red-400 rounded-md p-3">
      <div className="text-red-500  font-bold">Hurry up! Sale ends in:</div>
      <div className="flex items-center gap-1 text-white  ">
        <span className="bg-[#DD3327] block w-[90px] px-4 py-1 text-[10px]">
          <span className="text-lg font-bold">{time.days}</span> Days
        </span>
        <span className="bg-[#DD3327] block w-[90px] px-4 py-1 text-[10px]">
          <span className="text-lg font-bold">{time.hours}</span> Hrs
        </span>
        <span className="bg-[#DD3327] block w-[90px] px-4 py-1 text-[10px]">
          <span className="text-lg font-bold">{time.minutes}</span> Mins
        </span>
        <span className="bg-[#DD3327] block w-[90px] px-4 py-1 text-[10px]">
          <span className="text-lg font-bold">{time.seconds}</span> Secs
        </span>
      </div>
    </div>
  );
